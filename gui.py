    ...
    def update_map(self, slam_map: np.ndarray = None, 
        ...
        if slam_map is not None:
            ...
            if drone_pose:
                ...
                # Drone avec orientation
                cv2.circle(map_img, (dx, dy), 10, (0, 255, 255), -1)
                cv2.circle(map_img, (dx, dy), 10, (0, 200, 200), 2)
                
                # Flèche d'orientation
                angle = 0  # TODO: extraire de la rotation
                arrow_len = 15
                ax = int(dx + arrow_len * np.cos(angle))
                ay = int(dy - arrow_len * np.sin(angle))
                cv2.arrowedLine(map_img, (dx, dy), (ax, ay), (255, 255, 255), 2)
